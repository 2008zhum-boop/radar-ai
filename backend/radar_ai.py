...
def generate_smart_outline(title, angle, context_info=""):
    ...
    生成高质量大纲
    :param title: 文章标题
    :param angle: 策略角度 (如：借势营销、危机预警)
    :param context_info: 上下文信息 (如：相关新闻摘要)
    """
    # 模拟 AI 思考时间
    time.sleep(1) 
    
    # 1. 确定模板类型
    # 如果 angle 包含关键词，就用对应模板，否则用默认
    template_key = "默认通用"
...
# ... (保留之前的 TEMPLATES 和 generate_smart_outline)
...
def generate_full_article(title, outline, context_info=""):
    ...
    根据大纲生成 1000 字文章
    :param title: 文章标题
    :param outline: 大纲结构列表 [{'title': '...', 'sub_points': [...]}, ...]
    :param context_info: 上下文素材 (真实新闻摘要)
    """
    
    # 模拟 AI 写作过程，构建一篇完整的文章
    # 实际生产中，这里会多次调用 LLM，针对每个段落进行扩写
    ...
    intro = f"""
【导语】
{context_info[:50]}... 当下，"{title}" 正成为全网热议的焦点。这不仅是一次简单的热点事件，更折射出行业深层的变革逻辑。本文将透过现象看本质，为您深度复盘这一事件背后的商业脉络。
"""
    ...
    for section in outline:
        ...
        points = section['sub_points']
        
        # 模拟根据不同的大纲类型生成不同的段落内容
        paragraph = f"\n### {section_title}\n\n"
        
        # 扩写子要点 1
        if len(points) > 0:
            paragraph += f"首先，我们需要关注的是{points[0].replace('引用', '可以看到')}。据相关数据显示，这一趋势并非偶然，而是长期积累的结果。在当前的{context_info[:10]}背景下，这种变化显得尤为关键。\n\n"
        ...
        # 补充深度分析 (凑字数，增加专业感)
        paragraph += f"从更宏观的视角来看，{section_title.split('】')[1] if '】' in section_title else section_title} 实际上反映了市场对于确定性的渴求。无论是资本端还是用户端，大家都在用脚投票。这也解释了为什么在{context_info[:5]}发生后，舆论会呈现出如此一边倒的态势。\n"
